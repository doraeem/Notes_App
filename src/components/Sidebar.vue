  ...
  <aside class="sidebar">
    ...
    <ul>
      <li v-for="(note, index) in filteredNotes" :key="note.id"
        ...
        <div class="note-title-wrapper" @click="handleSelect(note.id)">
          <input v-if="renamingNoteId === note.id"
                 v-model="renameInput"
                 @keyup.enter="confirmRename(note.id)"
                 @blur="cancelRename"
                 class="rename-input"
                 autofocus />
          <span v-else @dblclick="startRename(note)">
        ...
        <div class="menu-wrapper">
          <button class="menu-btn" @click.stop="toggleMenu(note.id)">⋮</button>
          <div v-if="openMenuId === note.id" class="menu-dropdown" @click.stop>
...
function handleSelect(noteId) {
  if(openMenuId.value === noteId) return
